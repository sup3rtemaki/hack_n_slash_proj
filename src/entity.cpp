...
void Entity::draw(const RenderContext& context) {
	//draws current frame
	if (currentFrame != NULL && animSet != NULL && active) {
		currentFrame->Draw(animSet->spriteSheet, x - context.camera.x, y - context.camera.y, context);
	}

	//draw collision box
	if (solid && context.debugging) {
		// Criar rect tempor�rio ajustado pela c�mera
		...
		SDL_RenderDrawRect(context.renderer, &screenCollisionBox);  // usar rect ajustado
		SDL_SetRenderDrawColor(context.renderer, 0, 0, 0, SDL_ALPHA_OPAQUE);
		SDL_RenderDrawPoint(context.renderer, x - context.camera.x, y - context.camera.y);  // ajustar ponto tamb�m
	}
}
	...
	for (auto entity = entityList->begin(); entity != entityList->end();) {
		...
		if (deleteEntities) {
			...
			try {
				...
				throw; // Re-lan�a exce��o
			}
			catch (...) {
				cout << " CRASH (exce��o desconhecida)!" << endl;
