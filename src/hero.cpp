...
Hero::~Hero() {
	// PASSO 1: Limpar listas de refer�ncias PRIMEIRO
	// (nearItems s� tem ponteiros, n�o ownership)
	nearItems.clear();
	...
	// PASSO 4: Resetar ponteiros (n�o deletar - s�o gerenciados por outros)
	nearestDoor = nullptr;
	nearestCheckpoint = nullptr;
	nearestBloodstain = nullptr;
	currentMap = nullptr;
...
void Hero::updateAnimation() {
	...
	if (state == (int)HERO_STATE::RESTING) {
		// TODO: Fazer anim e l�gica de sentar na fogueira depois de implementar os menus
...
void Hero::addItemToInventory(std::unique_ptr<Item> item) {
	...
	// Procurar se item j� existe no invent�rio
	for (auto& [id, invItem] : inventory) {
		if (id == item->id) {
			// Item existe, s� aumentar quantidade
			invItem->quantity += item->quantity;
			// item ser� destru�do automaticamente ao sair da fun��o
			return;
		}
	}

	// Item novo, adicionar ao invent�rio
	int itemId = item->id;
...
void Hero::addItemToQuickAccess(int itemId, int position) {
	...
	if (position < 0 || position >= quickAccessInventory.size()) {
		...
		for (position; position < quickAccessInventory.size(); position++) {
			...
			if (quickAccessInventory[position] == -1) { // Achou proxima posi��o valida
				break; // Sai do loop pra guardar a posi��o
			}
		}
	}

	if (position == quickAccessInventory.size()) { // quickAccessInventory est� cheio
		position = 0;
	}
...
void Hero::useSelectedItem(int invIndex) {
	if (hp > 0 && (state == (int)HERO_STATE::MOVE || state == (int)HERO_STATE::IDLE)) {
		...
		if (item->quantity <= 0) {
			// Item acabou, remover do invent�rio
